...
def boar_brawl(player_score, opponent_score):
    ...
    if n != 0:
        ...
        n = 0
    return max(1, 3*abs(n - player_score%10))
    # END PROBLEM 2
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def strategy(my_score, opponent_score):
        return n 
    return strategy
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    initial_dice = strategy(0, 0)
    for i in range(goal):
        for j in range(goal):
            if strategy(i, j) != initial_dice:
                return False
    return True
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    max, max_scoring_num_rolls = 0, 0
    for i in range(1, 11):
        sum= 0
        for j in range ( 0 , samples_count):
            sum += roll_dice(i, dice)
        if sum > max:
            max, max_scoring_num_rolls = sum , i 
